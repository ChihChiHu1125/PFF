% Mapping Particle Filter - L96 Model (absoulte obs operator)
...
nt       = 20;  % number of integration timestep
...
alpha    = 0.05;         % the tuning parameter for the covariance of the kernel
r_influ  = 4;

% settings for MPF
stop_cri        = 1e-3;  % stopping criterion for the iteration
max_pseudo_step = 10;   % maximum number of iterations
...
inflation_fac   = 1;    % inflation for the kernel posterior
...
if io_obs == 1
    ...
    obs      = max(H_abs( Xt(:,warm_nt + t+1), obs_den, 1) + obs_rnd(:,obs_time),0); 
...
HT    = H_abs(mean(X_tmp,2),obs_den,2)';
...
for i=1:np % the "i" is to iterate for all particles
    if io_gauss_prior == 0 % gaussian mixture prior
        ...
        for j=1:np % the "j" is for the summation        
            numer(:,i) = numer(:,i) + (pseudo_X(:,j,1)) * exp( - (expon(j)-min_expon) );%分子
            denom(i)   = denom(i) + exp( - (expon(j)-min_expon) );%分母
        end
    ...
    HT = H_abs(pseudo_X(:,i,s),obs_den,2)';
    p_obs(:,i) = HT/R*(obs-H_abs(pseudo_X(:,i,s),obs_den,1));
...
%{
    if (norm_grad_KL(obs_time,s) > reduce_eps_cri)&&(s==1) % DA cycle往回8步
        reduce_eps_cri = norm_grad_KL(obs_time,s)
    elseif norm_grad_KL(obs_time,s) > reduce_eps_cri % DA cycle往回8步
        change_of_eps = 1;
        eps = eps/2;
        s=max(s-8,1);
...
% new convergence algorithm:
    ...
    if (norm_grad_KL(obs_time,s) > reduce_eps_cri)&&(s==1) % DA cycle往回8步
        reduce_eps_cri = norm_grad_KL(obs_time,s)
    elseif (eps<1e-6) % DA cycle往回8步
%        norm_grad_KL(obs_time,s) = 0;
%        s=s+1
        break
        %{
    elseif (sum(io_update(:,s))==0) && (sum(io_update(:,max(s-1,1)))~=0) && (norm_grad_KL(obs_time,s) <= 1.005*max(norm_grad_KL(obs_time,s-1:s-1))) % DA cycle往回8步
        change_of_eps = 1;
        eps = eps*100;
        ...
        ct=0;
        %}
    elseif norm_grad_KL(obs_time,s) > 10*reduce_eps_cri % DA cycle往回8步
        change_of_eps = 1;
        eps = eps*0.3;
        s=max(s-1,1);
        disp(['The eps has been changed to ',num2str(eps),' ... Redo the MPF based on this eps'])
        ct=0;
    elseif norm_grad_KL(obs_time,s) > 2.5*reduce_eps_cri % DA cycle往回8步
        change_of_eps = 1;
        eps = eps*0.5;
        s=max(s-1,1);
        disp(['The eps has been changed to ',num2str(eps),' ... Redo the MPF based on this eps'])
    ...
    elseif (s>=2) && (norm_grad_KL(obs_time,s) > 1.001*max(norm_grad_KL(obs_time,s-1:s-1))) % DA cycle往回8步
        change_of_eps = 1;
        eps = eps*0.8;
        s=max(s-1,1);
        disp(['The eps has been changed to ',num2str(eps),' ... Redo the MPF based on this eps'])
    ...
    elseif (ct>=20) && (norm_grad_KL(obs_time,s) <= 1.001*max(norm_grad_KL(obs_time,s-1:s-1))) % DA cycle往回8步
        change_of_eps = 1;
        eps = eps*1.4;
