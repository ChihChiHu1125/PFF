...
r_influ  = 4;
inflation_fac = 1.3;            % alpha: inflation factor
...
%     X_out = X_in(obs_den:obs_den:dim,:).^2;
    X_out = exp(X_in(obs_den:obs_den:dim,:)/4);
...
%     end
    for i=1:dim/obs_den
        X_out(i,obs_den*i) = exp(X_in(obs_den*i)/4)/4;
    end
end
%}