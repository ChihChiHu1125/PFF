% Mapping Particle Filter - L96 Model (square obs operator)
...
R        = 1*eye(dim/obs_den);  % error covariance of observation (modify this based on the observation operator)
...
if io_obs == 1
    ...
    obs      = max(H_square( Xt(:,warm_nt + t+1), obs_den, 1) + obs_rnd(:,obs_time),0); 
...
HT    = H_square(mean(X_tmp,2),obs_den,2)';
...
for i=1:np % the "i" is to iterate for all particles
    ...
    HT = H_square(pseudo_X(:,i,s),obs_den,2)';
    p_obs(:,i) = HT/R*(obs-H_square(pseudo_X(:,i,s),obs_den,1));
