% Mapping Particle Filter - L96 Model (exp obs operator)
...
max_pseudo_step = 500;   % maximum number of iterations
eps_init        = 1e-3;   % chosen pseudo-timestep parameter 
reduce_eps_cri  = 10;   % test run before assign the value; can be different for different observations
change_of_eps   = 0;
cond_num        = -5;
...
R        = 0.01*eye(dim/obs_den);  % error covariance of observation (modify this based on the observation operator)
...
if io_obs == 1
    ...
    obs      = H_exp( Xt(:,warm_nt + t+1), obs_den, 1) + obs_rnd(:,obs_time); 
...
HT    = H_exp(mean(X_tmp,2),obs_den,2)';
...
for i=1:np % the "i" is to iterate for all particles
    ...
    HT = H_exp(pseudo_X(:,i,s),obs_den,2)';
    p_obs(:,i) = HT/R*(obs-H_exp(pseudo_X(:,i,s),obs_den,1));
